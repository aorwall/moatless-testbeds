...
def test_instance_image_key_property(sample_instance):
    test_spec = TestSpec.from_instance(sample_instance)
    assert test_spec.instance_image_key == f"sweb.eval.{test_spec.arch}.{test_spec.instance_id}:latest"

def test_get_instance_container_name(sample_instance):
    test_spec = TestSpec.from_instance(sample_instance)
    assert test_spec.get_instance_container_name() == f"sweb.eval.{test_spec.instance_id}"
    assert test_spec.get_instance_container_name("run1") == f"sweb.eval.{test_spec.instance_id}.run1"
